...
def osr_ackermann(lin_vel, ang_vel, cfg, device):
    ...
    # Calculating the turning radius of the front wheels
    r_FL = torch.sqrt(torch.pow(direction*turning_radius-d1,2)+torch.pow(d3,2))   # 무조건 양수
    r_FR = torch.sqrt(torch.pow(direction*turning_radius+d1,2)+torch.pow(d3,2))   # 무조건 양수
    r_ML = torch.abs(turn_direction*torch.abs(turning_radius)-d4)
    r_MR = torch.abs(turn_direction*torch.abs(turning_radius)+d4)
    r_RL = torch.sqrt(torch.pow(direction*turning_radius-d1,2)+torch.pow(d2,2))   # 무조건 양수
    r_RR = torch.sqrt(torch.pow(direction*turning_radius+d1,2)+torch.pow(d2,2))   # 무조건 양수
    ...
    # Set velocities for point turn, else
    # if ang_vel is 0, wheel velocity is equal to linear velocity
    vel_FL = torch.where(torch.abs(turning_radius) < minimum_radius,
                         -torch.sqrt(torch.pow(d1,2)+torch.pow(d3,2))*ang_vel,  # 제자리 회전!
                         torch.where(ang_vel == 0, lin_vel, direction*turn_direction*r_FL*ang_vel))  # 제자리 회전이 아님!
    vel_FR = torch.where(torch.abs(turning_radius) < minimum_radius,
                         torch.sqrt(torch.pow(d1,2)+torch.pow(d3,2))*ang_vel,  # 제자리 회전!
                         torch.where(ang_vel == 0, lin_vel, direction*turn_direction*r_FR*ang_vel))  # 제자리 회전이 아님!
    vel_ML = torch.where(torch.abs(turning_radius) < minimum_radius,
                         -d4*ang_vel,  # 제자리 회전!
                         torch.where(ang_vel == 0, lin_vel, direction*torch.abs(r_ML*ang_vel)))  # 제자리 회전이 아님!
    vel_MR = torch.where(torch.abs(turning_radius) < minimum_radius,
                         d4*ang_vel,  # 제자리 회전!
                         torch.where(ang_vel == 0, lin_vel, direction*torch.abs(r_MR*ang_vel)))  # 제자리 회전이 아님!
    vel_RL = torch.where(torch.abs(turning_radius) < minimum_radius,
                         -torch.sqrt(torch.pow(d1,2)+torch.pow(d2,2))*ang_vel,  # 제자리 회전!
                         torch.where(ang_vel == 0, lin_vel, direction*turn_direction*r_RL*ang_vel))  # 제자리 회전이 아님!
    ...
    # set steering angles for point turn, else
    # ackermann
    theta_FL = torch.where(torch.abs(turning_radius) < minimum_radius,
                           -torch.atan2(d3,d1),
                           turn_direction*torch.atan2(d3, torch.abs(turning_radius-d1)))        # case 1
    theta_FR = torch.where(torch.abs(turning_radius) < minimum_radius,
                           torch.atan2(d3,d1),
                           turn_direction*torch.atan2(d3, torch.abs(turning_radius+d1)))        # case 1
    theta_RL = torch.where(torch.abs(turning_radius) < minimum_radius,
                           torch.atan2(d2,d1),
