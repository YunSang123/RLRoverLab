...
#################################
# JYS : /workspace/isaac_rover에 있는 skrl 디렉터리를 강제 로드함
import importlib.util

# /workspace/isaac_rover 경로의 skrl 강제 로드
local_skrl_path = "/workspace/isaac_rover/skrl"
spec = importlib.util.spec_from_file_location("skrl", f"{local_skrl_path}/__init__.py")
skrl = importlib.util.module_from_spec(spec)
sys.modules["skrl"] = skrl
spec.loader.exec_module(skrl)
#################################
...
def train():
    ...
    observation_space = gym.spaces.Box(low=-math.inf, high=math.inf, shape=(num_obs,))
    action_space = gym.spaces.Box(low=-1.0, high=1.0, shape=(num_actions,))
    print(f"Observation space_shape: {observation_space.shape}")
    print(f"Observation space: {observation_space}")
    print(f"Observation type: {type(observation_space)}")
    ...
    if args_cli.load:   # 기존 model을 이어서 학습
        ...
        checkpoint_path = "./load/osr_440k.pt"
