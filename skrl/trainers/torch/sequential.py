...
class SequentialTrainer(Trainer):
    ...
    def train(self) -> None:
        ...
        for timestep in tqdm.tqdm(range(self.initial_timestep, self.timesteps), disable=self.disable_progressbar, file=sys.stdout):
            ...
            # compute actions
            """
            모든 agent가 현재 state를 기반으로 action을 수행
            torch.no_grad(): 학습이 아닌 예측 단계로, 그래프를 저장하지 않아 메모리 사용 절약.
            """
            with torch.no_grad():
                # self.agents : 에이전트들의 리스트
                # self.agents_scope : 각 에이전트가 처리할 상태 범위를 나타내는 범위 정보
                # 
            ...
            # post-interaction
            # 각 에이전트가 상호작용 후 필요한 작업(학습 등)을 수행
            for agent in self.agents:
